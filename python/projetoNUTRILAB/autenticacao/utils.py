...
def password_is_valid(request, password, confirm_password):
    ...
    if not re.search('[A-Z]', password):
        messages.add_message(request, constants.ERROR, 'Suas senhas não contem letras maiúsculas')
        return False
    ...
    if not re.search('[1-9]', password): #if not verifica se a condição seja satisfeita, caso não seja ira utilizar message do Django para retornar uma mensagem
